...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="row">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('update.blotter') }}" method="post" class="row">
                    @csrf
                ...
                <button class="btn btn-primary" type="submit">Submit</button>
                    ...
                    <div class="card">
                        <div class="card-body row">
                            ...
                            <div class="form-group col-md-3">
                                <label class="">Entry Number</label>
                                <input type="number" name="entry_number" class="form-control" value="{{ $blotter->entry_number }}" readonly />
                                <span class="text-danger">{{ $errors->first('entry_number') }}</span>

                            </div>
                            <div class="form-group col-md-3">
                                <label class="">Time</label>
                                <input type="time" name="time" class="form-control" value="{{ $blotter->time }}" readonly />
                                <span class="text-danger">{{ $errors->first('time') }}</span>

                            </div>
                            <div class="form-group col-md-3">
                                <label class="">Officers:</label>
                                <select name="officer_id" class="form-control" disabled>
                                    <option value="">Select Officer</option>
                                    @foreach ($officers as $officer)
                                        <option value="{{ $officer->id }}"
                                        @if (!empty($blotter) && $blotter->officer_id == $officer->id) selected @endif>{{ $officer->full_name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-md-3">
                                <label class="">Incident Type</label>
                                <select name="incident_type_id" id="" class="form-control" disabled>
                                    <option value="">Select Incident Type</option>
                                    @foreach ($incidents as $incident)
                                        <option value="{{ $incident->id }}"
                                        @if (!empty($blotter) && $blotter->incident_type_id == $incident->id) selected @endif>{{ $incident->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label class="">Job Final </label>
                                <select name="job_final_id" id="" class="form-control" disabled>
                                    <option value="">Select Job Final</option>
                                    @foreach ($jobs as $job)
                                        <option value="{{ $job->id }}"
                                        @if (!empty($blotter) && $blotter->job_final_id == $job->id) selected @endif>{{ $job->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label class="">Crimes</label>
                                <select name="crime_id" id="" class="form-control" disabled>
                                    <option value="">Select Crime</option>
                                    @foreach ($crimes as $crime)
                                        <option value="{{ $crime->id }}"
                                        @if (!empty($blotter) && $blotter->crime_id == $crime->id) selected @endif>{{ $crime->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label class="">Requests</label>
                                <select name="person_request_id" id="" class="form-control" disabled>
                                    <option value="">Select Request</option>
                                    @foreach ($persons as $person)
                                        <option value="{{ $person->id }}"
                                        @if (!empty($blotter) && $blotter->person_request_id == $person->id) selected @endif>{{ $person->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label class="">Medical Requests</label>
                                <select name="medical_request_id" id="" class="form-control" disabled>
                                    <option value="">Select Medical Request</option>
                                    @foreach ($medicals as $medical)
                                        <option value="{{ $medical->id }}"
                                        @if (!empty($blotter) && $blotter->medical_request_id == $medical->id) selected @endif>{{ $medical->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label class="">Codes/Alarms</label>
                                <select name="code_alarm_id" id="" class="form-control" disabled>
                                    <option value="">Select Codes/Alarms</option>
                                    @foreach ($codes as $code)
                                        <option value="{{ $code->id }}"
                                        @if (!empty($blotter) && $blotter->code_alarm_id == $code->id) selected @endif>{{ $code->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label class="">Other</label>
                                <select name="other_id" id="" class="form-control" disabled>
                                    <option value="">Select Other</option>
                                    @foreach ($others as $other)
                                        <option value="{{ $other->id }}"
                                        @if (!empty($blotter) && $blotter->other_id == $other->id) selected @endif>{{ $other->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label class="">Building</label>
                                <select name="building_id" id="" class="form-control" disabled>
                                    <option value="">Select Building</option>
                                    @foreach ($buildings as $building)
                                        <option value="{{ $building->id }}"
                                        @if (!empty($blotter) && $blotter->building_id == $building->id) selected @endif>{{ $building->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label class="">Parking Lot</label>
                                <select name="parking_lot_id" id="" class="form-control" disabled>
                                    <option value="">Select Parking Lot</option>
                                    @foreach ($parkings as $parking)
                                        <option value="{{ $parking->id }}"
                                        @if (!empty($blotter) && $blotter->parking_lot_id == $parking->id) selected @endif>{{ $parking->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label class="">Area</label>
                                <select name="area_id" id="" class="form-control" disabled>
                                    <option value="">Select Area</option>
                                    @foreach ($areas as $area)
                                        <option value="{{ $area->id }}"
                                        @if (!empty($blotter) && $blotter->area_id == $area->id) selected @endif>{{ $area->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-12">
                                <label class="">Comments</label>
                                <textarea type="text" name="comment" class="form-control" readonly>
                                    {{ $blotter->comment }}
                                </textarea>
